	...
	\node[eqblock, above of=left, xshift=6cm] (eq2) {\large$a_t(s_t)$};
	\node[eqblock, above of=left, xshift=9cm] (eq3) {\large$u_t(s_t, a_t)$};
	\node[eqblock, above of=bottom_left, xshift=3cm] (eq4) {\large$\{u_{t + 1}(s_{t + 1}, a)\}_{a\in A}$};
	\node[eqblock, above of=bottom_left, xshift=6cm] (eq6) {\large$a_{t + 1}(s_{t + 1})$};
