	...
	\node[eqblock, above of=left, xshift=3cm] (eq1) {\large$\{u_t(s, a)\}_{a\in A}$};
	\node[eqblock, above of=left, xshift=6cm] (eq2) {\large$d_t(s)$};
	\node[eqblock, above of=left, xshift=9cm] (eq3) {\large$u_t(s, d_t(s))$};
	\node[eqblock, above of=bottom_left, xshift=3cm] (eq4) {\large$\{u_{t + 1}(s^\prime, a)\}_{a\in A}$};
	\node[eqblock, above of=bottom_left, xshift=6cm] (eq6) {\large$d_{t + 1}(s^\prime)$};
	\node[eqblock, above of=bottom_left, xshift=9cm] (eq5) {\large$u_{t + 1}(s^\prime, d_{t + 1}(s^\prime))$};
