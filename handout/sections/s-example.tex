...
%-------------------------------------------------------------------------------
We now turn to the exemplifying analysis of a canonical EKW model on human capital investment analyzed in \citet{Keane.1997}. This provides us with a well-known and empirically motivated setting. We first outline the basic setup, provide some descriptive statistics of the empirical data used for the calibration, and then explore the economic insights provided by the model.
%-------------------------------------------------------------------------------
\subsection{The career decisions of young men}
%-------------------------------------------------------------------------------
%
% What are core differences to Appendix given the target audience?
...
\end{align*}
%
\paragraph{Exemplary discussion} As an example, we only provide the exact specification for the per-period utility in the white collar occupation. All details are available in Appendix \ref{Computational implementation}.\\
...
\end{align}
%
\noindent\textbf{White-collar} Equation (\ref{Skill-production function}) shows the parameterization of the deterministic component of the skill-production function.
%
...
%
\paragraph{Wrapping up} Work experience $\bm{k_t}$  and years of completed schooling $h_t$ evolve deterministically.
%
\begin{align*}
...
\subfloat[Average wage]{\scalebox{0.25}{\includegraphics{material/fig-data-wages-average-bw}}}
\begin{center}
...
\paragraph{Calibration} The model is calibrated using maximum likelihood estimation. Figure \ref{Model fit} shows overall agreement between the empirical data and a dataset simulated using the results from the calibration within the support of the data by age. On the left we show the choice probability by for working in a blue-collar occupation, while we plot the average wages on the right.

\begin{figure}[h!]\centering
\caption{Model fit}\label{Model fit}
\subfloat[Blue-collar]{\scalebox{0.25}{\includegraphics{material/fig-model-fit-blue-collar-bw}}}\hspace{0.3cm}
\subfloat[Average wage]{\scalebox{0.25}{\includegraphics{material/fig-model-fit-average-bw}}}
\end{figure}\FloatBarrier
