...
%-------------------------------------------------------------------------------
We now analyze a canonical EKW model.

\paragraph{Setting} We start with an outline and discussion of the occupational choice model presented in \citet{Keane.1997} to provide a well-known and empirically motivated setting. While the model is deliberately simple compared to more recent applications, it already contains all core components. We will first outline the basic setup and provide some basic descriptive statistics.\\

\noindent We conclude by sketching its specification, simulation, and calibration using our group's research codes \verb+respy+ and \verb+estimagic+.
%-------------------------------------------------------------------------------
\subsection{The career decisions of young men}
%-------------------------------------------------------------------------------
We follow individuals over their working life from young adulthood at age 16 to retirement at age 65. The decision period $t = 16, \dots, 65$  is a year.\\

\noindent Individuals are initially heterogeneous. They differ with respect to their initial level of completed schooling $h_1$ and have one of four different $\mathcal{J} = \{1, \hdots, 4\}$ alternative-specific skill endowments $\bm{e} = \left(e_{j,a}\right)_{\mathcal{J} \times \mathcal{A}}$.\\
...
\begin{Remark}
What are core differences to Appendix given the target audience. no references to econ papers, need to explain sheep-skin effects, maybe expected signs useful afeter all
\end{Remark}

\paragraph{Exemplary discussion} As an example, we only provide the exact specification for the per-period utlity in the white collar occuaption. All details are available in Appendix \ref{Computational implementation}

The wage component $w_{a}(\cdot)$ is given by the product of the market-equilibrium rental price $r_{a}$ and an occupation-specific skill level $x_{a}(\cdot)$. The overall level of human capital determines the latter.
%
\begin{align*}
...
%
\paragraph{Evolution of state variables} Work experience $\bm{k_t}$  and years of completed schooling $h_t$ evolve deterministically.
%
\begin{align*}
...
\paragraph{Calibration} The model is calibrated using maximum likelihood Estimation.

\begin{figure}[h!]\centering
\caption{Model fit}\label{Economic mechanism and policy forecast}
\subfloat[Choice]{\scalebox{0.25}{\includegraphics{material/fig-model-fit-blue-collar-bw}}}\hspace{0.3cm}
\subfloat[Wages]{\scalebox{0.25}{\includegraphics{material/fig-model-fit-mean-bw.png}}}
\end{figure}\FloatBarrier

The model reproduces the basic patterns in the data well.
...
\subfloat[Time preference]{\scalebox{0.25}{\includegraphics{material/fig-economic-mechanisms-bw}}}\hspace{0.3cm}
\subfloat[Tuition subsidy]{\scalebox{0.25}{\includegraphics{material/fig-policy-forecast-bw}}}
\end{figure}\FloatBarrier
...
\subfloat[Parameterization]{\scalebox{0.4}{\includegraphics{params}}}\hspace{0.3cm}
\subfloat[Options]{\scalebox{0.4}{\includegraphics{options}}}
\end{figure}\FloatBarrier
