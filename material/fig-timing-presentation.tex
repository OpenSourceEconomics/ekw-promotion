	%!TEX root = ../main.tex
	\begin{tikzpicture}[scale=1.0, every node/.style={scale=0.7}]
	...
	\node[eqblock, above of=left, xshift=3cm] (eq1) {\large$\{r_t(s_t, a_t)\}_{a_t\in A}$};
	\node[eqblock, above of=left, xshift=6cm] (eq2) {\large$a_t$};
	\node[eqblock, above of=left, xshift=9cm] (eq3) {\large$r_t(s_t, a_t)$};
	\node[eqblock, above of=bottom_left, xshift=2cm] (eq4) {\large$\{r_{t + 1}(s_{t + 1}, a_{t + 1}\noindent )\}_{a_{t + 1}\in A}$};
	\node[eqblock, above of=bottom_left, xshift=6cm] (eq6) {\large$a_{t + 1}$};
	\node[eqblock, above of=bottom_left, xshift=9cm] (eq5) {\large$r_{t + 1}(s_{t + 1}, a_{t + 1})$};
